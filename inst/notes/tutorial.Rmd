...
## The net benefit for 'opt-out' vs. 'opt-in' policies 

For many clinical contexts, a risk model is used in one of two ways. In the more common setting, the standard-of-care for a population is to assign a treatment to no one. Clinicians then use a model to categorize patients as 'high-risk', and have the potential to treat those patients with alternative care. This type of policy is referred to as an 'opt-in' policy. The net benefit for such a policy is defined in terms of the true and false positive rates, the outcome prevalence $\rho$, and the 'high-risk' threshold $r_H$: 

$$ 
NB^{(opt-in)}(r_H) = \rho TPR(r_H) - ({1-\rho})\Big(\frac{r_H}{1-r_H}\Big)FPR(r_H)
$$

Alternatively, one could imagine the standard-of-care to be to assign a treatment to an entire population. The use of a model in this setting would be to identify observations as 'low-risk', so that those patients may 'opt-out' of treatment. The net benefit for this type of policy is then defined in terms of the true and false negative rates, and the 'low-risk' threshold $r_L$: 

$$ 
NB^{(opt-out)}(r_H) = (1-\rho) TNR(r_L) - {\rho}\Big(\frac{1-r_L}{r_L}\Big)FNR(r_L)
$$

So far in the tutorial we have been calculating the net benefit curves using the default 'opt-in' policy. To calculate the 'opt-out' net benefit, we set `policy = 'opt-out'`: 
